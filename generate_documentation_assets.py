"""
generate_documentation_assets.py - Generatore automatico di figure e grafici 
...
per generare i documenti basta mandare il comando da terminale:
    python generate_documentation_assets.py
"""
...
def generate_all_figures():
    ...
    df_merged = df.merge(df_logic, on=[config.ID_COL, config.TIME_COL])
    ...
    boxes = [
        {"text": "1. Telemetria SCADA\n& Ingegnerizzazione Dati\n(data_loader.py)", "xy": (0.1, 0.5), "color": "#dbeafe", "edge": "#2563eb"},
        ...
        {"text": "4. Ottimizzatore CSP\nPianificazione Risorse\n(maintenance_optimizer.py)", "xy": (0.75, 0.5), "color": "#dcfce7", "edge": "#16a34a"},
    ]
    ...
    ax.axvline(config.CRITICAL_RUL_THRESHOLD, color="red", linestyle="--", label="Soglia Critica (100h)")
    ax.axvline(config.WARNING_RUL_THRESHOLD, color="orange", linestyle="--", label="Soglia Warning (300h)")
    ...
    # -------------------------------------------------------------
    fig, ax = plt.subplots(figsize=(9, 4.5))
    sample_turbines = df[config.ID_COL].unique()[:5]
    for t_id in sample_turbines:
        sub = df[df[config.ID_COL] == t_id]
        ax.plot(sub[config.TIME_COL], sub["RUL"], label=t_id, lw=1.8)
    ...
    single_t = df[df[config.ID_COL] == sample_turbines[0]]
    
    ax1.plot(single_t[config.TIME_COL], single_t["gearbox_oil_temp_c"], color="#e11d48", label="Temp Olio (°C)")
    ax1.axhline(config.PHYSICAL_THRESHOLDS["gearbox_oil_max_safe"], color="orange", linestyle=":", label="Soglia Safe")
    ax1.axhline(config.PHYSICAL_THRESHOLDS["gearbox_oil_alarm"], color="red", linestyle="--", label="Allarme Critico")
    ax1.set_ylabel("Temperatura (°C)")
    ax1.set_title(f"Monitoraggio Termico e Z-Score Anomalie ({sample_turbines[0]})")
    ax1.legend(loc="upper left")
    
    ax2.plot(single_t[config.TIME_COL], single_t["gearbox_oil_temp_c_zscore"], color="#7c3aed", label="Rolling Z-Score")
    ax2.axhline(config.ZSCORE_THRESHOLD, color="black", linestyle="--", label=f"Soglia Anomalia ({config.ZSCORE_THRESHOLD}σ)")
    ...
    # -------------------------------------------------------------
    fig, ax = plt.subplots(figsize=(9, 4.5))
    single_res = df_merged[df_merged[config.ID_COL] == sample_turbines[0]]
    ax.plot(single_res[config.TIME_COL], single_res["failure_risk_score"], label="Failure Risk Score (BN)", color="#ef4444", lw=2)
    ax.plot(single_res[config.TIME_COL], single_res["health_state"] / 2.0, label="Stato Reale Normalizzato", color="#3b82f6", linestyle="--")
    ax.set_xlabel("Ore di Funzionamento (h)")
    ...
    # -------------------------------------------------------------
    opt = maintenance_optimizer.WindFarmMaintenanceOptimizer(planning_days=10, max_crews=3)
    ...
    print("\n[OK] Tutti gli asset grafici sono stati generati con successo in 'figures_doc/'.")
