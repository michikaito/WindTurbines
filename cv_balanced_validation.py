...
def run_model_comparison(
    ...
    print(f"Turbine uniche: {df[config.ID_COL].nunique()}", flush=True)

    if max_rows is not None and len(df) > max_rows:
        print(f"[INFO] Campionamento veloce: tengo {max_rows} righe su {len(df)}", flush=True)
        df = df.sort_values([config.ID_COL, config.TIME_COL]).groupby(config.ID_COL, group_keys=False).head(
            max(1, max_rows // df[config.ID_COL].nunique())
        )
    ...
    df["hydraulic_state"] = (df["hydraulic_pressure_bar"] < config.PHYSICAL_THRESHOLDS["hydraulic_min_safe"]).astype(str)
    ...
    groups = df[config.ID_COL].values
    ...
    out_path = config.RESULTS_DIR / "model_comparison_wind_turbines.csv"
