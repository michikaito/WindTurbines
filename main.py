...
def run_pipeline():
    ...
    print(f"      - Turbine monitorate: {df[config.ID_COL].nunique()}")
    print(f"      - Orizzonte temporale max: {df[config.TIME_COL].max()} ore\n")
    ...
    num_critical_events = (df_logic["kb_predicted_state"] == config.HEALTH_STATES["CRITICAL"]).sum()
    ...
    df_combined = df.merge(df_logic, on=[config.ID_COL, config.TIME_COL])
    ...
    optimizer = maintenance_optimizer.WindFarmMaintenanceOptimizer(
        planning_days=config.CSP_CONFIG["planning_horizon_days"],
        max_crews=config.CSP_CONFIG["max_crews_available"]
    )
    ...
    # ------------------------------------------------------------------
    output_csv = config.RESULTS_DIR / "pipeline_execution_summary.csv"
    df_combined.to_csv(output_csv, index=False)
    
    if schedule:
        schedule_csv = config.RESULTS_DIR / "maintenance_schedule_plan.csv"
